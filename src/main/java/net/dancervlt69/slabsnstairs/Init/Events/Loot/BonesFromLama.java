...
public class BonesFromLama extends LootModifier {
    ...
    protected BonesFromLama(ILootCondition[] conditionsIn, Item item) {
        super(conditionsIn);
        this.item = item;
    }
    ...
    public static class Serializer extends GlobalLootModifierSerializer<BonesFromLama> {
        @Override
        public BonesFromLama read(ResourceLocation name, JsonObject object, ILootCondition[] conditionIn) {
            Item item = ForgeRegistries.ITEMS.getValue(
                    new ResourceLocation(JSONUtils.getAsString(object, "item")));
            return new BonesFromLama(conditionIn, item);
        }
        @Override
        public JsonObject write(BonesFromLama instance) {
