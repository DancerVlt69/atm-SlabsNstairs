...
public class BonesFromChicken extends LootModifier {
    ...
    protected BonesFromChicken(ILootCondition[] conditionsIn, Item item) {
        super(conditionsIn);
        this.item = item;
    }
    ...
    public static class Serializer extends GlobalLootModifierSerializer<BonesFromChicken> {
        @Override
        public BonesFromChicken read(ResourceLocation name, JsonObject object, ILootCondition[] conditionIn) {
            Item item = ForgeRegistries.ITEMS.getValue(
                    new ResourceLocation(JSONUtils.getAsString(object, "item")));
            return new BonesFromChicken(conditionIn, item);
        }
        @Override
        public JsonObject write(BonesFromChicken instance) {
