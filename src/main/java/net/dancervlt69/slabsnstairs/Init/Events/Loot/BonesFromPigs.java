...
public class BonesFromPigs extends LootModifier {
    ...
    protected BonesFromPigs(ILootCondition[] conditionsIn, Item item) {
        super(conditionsIn);
        this.item = item;
    }
    ...
    public static class Serializer extends GlobalLootModifierSerializer<BonesFromPigs> {
        @Override
        public BonesFromPigs read(ResourceLocation name, JsonObject object, ILootCondition[] conditionIn) {
            Item item = ForgeRegistries.ITEMS.getValue(
                    new ResourceLocation(JSONUtils.getAsString(object, "item")));
            return new BonesFromPigs(conditionIn, item);
        }
        @Override
        public JsonObject write(BonesFromPigs instance) {
