...
public class BerriesFromTallGrass extends LootModifier {
    ...
    protected BerriesFromTallGrass(ILootCondition[] conditionsIn, Item item) {
        super(conditionsIn);
        this.item = item;
    }
    ...
    protected List<ItemStack> doApply(List<ItemStack> generatedLoot, LootContext context) {
        if(context.getRandom().nextFloat() > 0.925F) {
    ...
    public static class Serializer extends GlobalLootModifierSerializer<BerriesFromTallGrass> {
        @Override
        public BerriesFromTallGrass read(ResourceLocation name, JsonObject object, ILootCondition[] conditionIn) {
    ...
    public static void getItemFromTagList(Ingredient.TagList tagList) {
